...
import ch.fastforward.magnolia.crud.pages.CRUDPage;
...
/**
 * PageMVCHandler subclass which offerst the common shop tasks like adding a
 * product to the cart, deleting a cart item or saving the cart.
 * 
 * @author will
 */
public class ShopPage extends CRUDPage {
  ...
  // is beign saved
  private Boolean sendConfirmation = new Boolean(false);
  ...
  /**
   * Adds the product whos UUID is stored in the "product" query parameter to
  ...
  public String saveAndConfirmOrder() {
    ...
    this.setBeanType(NodeDataUtil.getString(shopConfigNode, "cartBeanType"));
    this.setBeanLocation(NodeDataUtil.getString(shopConfigNode, "cartSessionVariable"));
    view = updateNode();
    if (!view.equals(VIEW_SUCCESS)) {
      return view;
    }
    ...
    session.removeAttribute(getBeanLocation());
    return VIEW_SUCCESS;
  ...
  public ShoppingCart getShoppingCart(Content shopConfigNode) {
    ...
    if (shopConfigNode != null) {
      ...
      if (StringUtils.isBlank(getBeanLocation())) {
        setBeanLocation(shopConfigNode.getNodeData("cartSessionVariable").getString());
      }
      if (StringUtils.isBlank(getBeanLocation())) {
        return null;
      }
      ...
      // not exist)
      cart = (ShoppingCart) request.getSession().getAttribute(getBeanLocation());
      if (cart == null) {
        log.debug("No ShoppingCart object found at " + getBeanLocation());
        ...
        try {
          ...
          request.getSession().setAttribute(getBeanLocation(), cart);
