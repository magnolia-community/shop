...
public class DefaultShoppingCart extends MgnlDataBean implements ShoppingCart, Serializable {
  ...
  public DefaultShoppingCart(Content priceCategory) {
    super();
  ...
  public int addToShoppingCart(String productUUID, int quantity) {
    ...
    if (productUUID != null) {
      ...
      if (indexOfProductInCart >= 0) {
        ...
        if (!matching.isEmpty()) {
          Content priceNode = matching.iterator().next();
  ...
  /**
   * Validates parts of the shopping cart depending on the key passed in.
   * 
   * @param key
   * @return a hash with the error message with the keys being used to identify
   *         the field which produced the error and the value to identify the
   *         error type, e.g. {"orderAddressLastname":
   *         "validation.failed.empty"}
   * @todo Implement the other validation sets (e.g. validateShippingAddress())
   * @todo Instead of hardcoding the validation in the cart class a flexible
   *       (i.e. configurable) validation mechanism would be good. The
   *       validation sets could be configured as nodes with subnodes for each
   *       field to validate containing a node data for the field name and a
  ...
  public Map validate(String key) {
    if (StringUtils.isBlank(key)) {
      return null;
    } else if (key.equals("orderAddress")) {
  ...
  private Map validateOrderAddress() {
    HashMap errors = new HashMap();

    // lastname may not be empty
    ValidationUtil.validateNotEmpty("orderAddressLastname", getOrderAddressLastname(), errors);

    // adress may not be empty
    ValidationUtil.validateNotEmpty("orderAddressStreet", getOrderAddressStreet(), errors);

    // zip may not be empty
    ValidationUtil.validateNotEmpty("orderAddressZip", getOrderAddressZip(), errors);

    // city may not be empty
    ValidationUtil.validateNotEmpty("orderAddressCity", getOrderAddressCity(), errors);
